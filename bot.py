...
async def on_message(message):
    ...
    if dottie.user in message.mentions:
        responses = [
            ...
            f"Hi there! I'm an experimental Discord bot created by ||the cuddly bugs|| {', '.join(dottie.get_user(u).name for u OWNERS[:-1])} and {dottie.get_user(OWNERS[-1])}! My mission is to be something positive. :white_heart:"
        ]
...
async def on_guild_join(guild):
    ...
    embed = discord.Embed(colour=discord.Colour(pink_embed))
    embed.description = f"""Hi! I'm {dottie.user.name}, a test project by {', '.join(dottie.get_user(u).name for u OWNERS[:-1])}- with the help of {dottie.get_user(OWNERS[-1])} of course! :white_heart:\n
